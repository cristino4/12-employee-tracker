...
class Employee {
    ...
    async deleteEmployee(param,identifier){
        try {
            if(param === 'all'){
                this.db.query(clearTable);
                l.debug('MySQL: cleared table employee');
            } else{
                const query = `DELETE FROM employee WHERE ${param}='${identifier}'`
                this.db.query(query);
                l.debug(`MySQL: cleared employee entry with ${param} ${identifier}`);
            };
            return true;
        } catch (error) {
            l.debug(error);
